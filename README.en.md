...
| Command | Description |
|---------|-------------|
| `/sessions` or `/ls` | List active Claude Code sessions (`👀` = monitoring, actively reading messages; `📌 bound` = your messages go here; `📥 default` = plain messages go here). One paragraph per entry, like `○ [5] fintary:main 👀 📌 bound 📁 fintary/data-sync-field-mapping · 🤖 fintary-69 · ⏱ 3h ago`: 📁 is the directory Claude is currently in (a worktree as `repo/worktree`), 🤖 its Claude Code cross-session name, which is how same-named sessions are told apart; emoji serve as field labels because WeChat renders a single newline as a space. Ends with a labelled version paragraph, `📦 Running version: wechat-claude @ v1.2.0` plus a link to that version's npm page; a pre-release expands to `v1.2.0 pre-release (branch xxx · build #10 · commit 1e0b17f)` linking to that commit on GitHub. When npm has a newer `latest`, an upgrade notice follows (npm is asked at most every 6 hours; the last answer is reused if it can't be reached) |
