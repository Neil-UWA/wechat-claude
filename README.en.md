...
| Command | Description |
|---------|-------------|
| `/sessions` or `/ls` | List active Claude Code sessions (`👀` = monitoring, actively reading messages; `📌 bound` = your messages go here; `📥 default` = plain messages go here). One paragraph per entry, one field per line, sized for a phone screen: `○ [5] fintary:main 👀 📌 bound`, then `📁 fintary/data-sync-field-mapping` (the directory Claude is currently in, a worktree as `repo/worktree`), `🤖 fintary-69` (its Claude Code cross-session name) and `⏱ 3h ago`; same-named sessions are told apart by 📁 and 🤖. Emoji serve as field labels, so the text still reads when desktop WeChat collapses newlines to spaces. Ends with a labelled version paragraph, `📦 Running version: wechat-claude @ v1.2.0` (only x.y.z; a dev build's suffix is not shown). When npm has a newer `latest`, an upgrade notice follows (npm is asked at most every 6 hours; the last answer is reused if it can't be reached, and a failed attempt is not retried for 30 minutes) |
