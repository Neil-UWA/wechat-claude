...
| 命令 | 说明 |
|------|------|
| `/sessions` 或 `/ls` | 列出活跃的 Claude Code session（`👀` = 监控中，正在实时读取消息；`📌 已绑定` = 你的消息都发到这里；`📥 默认接收` = 不带前缀的消息发到这里）。每条一段，形如 `○【5】fintary:main 👀 📌 已绑定 📁 fintary/data-sync-field-mapping · 🤖 fintary-69 · ⏱ 3 小时前`：📁 是 Claude 当前所在目录（worktree 显示为 `仓库/worktree名`），🤖 是 Claude Code 跨会话名，同名 session 靠这两项区分；用 emoji 当字段标签是因为微信会把单个换行渲染成空格。末尾一段是当前版本标签：`📦 当前版本：wechat-claude @ v1.2.0` 加一个链接 —— 正式版指向该版本的 npm 页面，预发布构建指向 GitHub 上对应的提交（版本号只显示 x.y.z）。如果 npm 上有更新的 `latest`，再附一段升级提示（最多每 6 小时查一次 npm，查不到就沿用上次结果） |
