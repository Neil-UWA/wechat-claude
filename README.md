...
| `/s <编号> <消息>`<br>`/<编号> <消息>` | 按编号发消息给某个 session（`/3 你好` 是 `/s 3 你好` 的简写；简写只认纯数字，所以打错的命令不会被当成 session 名投递出去）。编号在 session 生命周期内固定不变 —— 其他 session 开启或关闭都不会让它移位（退役的编号不会被复用；所有 session 都消失后编号重新计数） |
...
- 图片回复也能引用（发图时的消息 id 一并记下），引用摘要显示为 `[图片]` 或你写的说明文字
- 引用 + `/s <编号>` 同时用也可以：目标听你的，引用的那句仍然会作为上下文带给它
- 如果写那条回复的 session 已经退出了，微信里会直接告诉你，并按默认路由投递

**是怎么认出来的**：微信发过来的引用只有一个 `ref_msg`，里面是被引用消息的 **id 和时间**，
**没有原文**。而发消息的接口会把新消息的 id 返回给我们 —— 于是 session 每次回复微信时，
MCP server 就把这个 id 连同发出去的内容记进 `outbox.json`（session id、名字、原文、消息
id，留 24 小时、最多 200 条），引用进来时按 id 反查是谁说的。
...
所以尾注关掉（`"replyFooter": false`）、或者某条回复是旧版本发的没记下 id，都还有兜底。

尾注认的是里面的**名字**，不是 `#编号`：编号虽然不会重复使用，但所有 session 都退出后
计数会从 1 重新开始，所以隔天的 `#3` 可能指向一个跟那条消息无关的新 session。名字对不上就
按"那个 session 已经退出"处理，不猜。时间兜底同理，只在这条回复根本没记下 id 时才用 ——
你引用自己发的消息时，不会被塞给当时刚好在说话的那个 session。

因为微信不带原文，投递给 session 的那句"你引用的是这条"摘要，是用 `outbox.json` 里记着的
原文还原出来的。
