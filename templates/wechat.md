...
7. When the Monitor emits an event:
   ...
   - **Relaying to another session.** Plain messages land in the default or bound session, so the user may be addressing a different one ("让 integration 跑一下测试", "tell the backend session to rebase"). If the message names a session, a repo/worktree, or a task that clearly belongs elsewhere, resolve the target from the `Active sessions` list in `wechat_status` — match on the WeChat routing name, the `SendMessage:` name, or the working directory — then forward with `SendMessage` using the name after `SendMessage:` on that row (never the routing name). Tell the user on WeChat which session you forwarded to and how to reach it directly next time (`/s <name> <msg>`), and relay the target's reply back to WeChat when it arrives. If more than one session could be meant, ask on WeChat instead of guessing.
